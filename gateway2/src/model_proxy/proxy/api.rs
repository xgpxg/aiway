...
use crate::model_proxy::proxy::response::ModelError;
...
/// 对话补全
pub async fn chat_completions(body: Option<Bytes>) -> Result<Response, ModelError> {
    if body.is_none() {
...
/// 文本转语音
pub async fn audio_speech(body: Option<Bytes>) -> Result<Response, ModelError> {
    if body.is_none() {
        return Err(ModelError::Parse("Request body is empty".to_string()));
    }
...
/// 创建图像
pub async fn images_generations(body: Option<Bytes>) -> Result<Response, ModelError> {
    if body.is_none() {
        return Err(ModelError::Parse("Request body is empty".to_string()));
    }
    ...
    match ModelFactory::get_provider(&req.model.clone().unwrap_or_default()) {
        Ok(provider) => Proxy::create_image(req, &provider).await,
        Err(e) => Err(e),
    }
}

/// 编辑图像
pub async fn images_edits(body: Option<Bytes>) -> Result<Response, ModelError> {
    if body.is_none() {
        return Err(ModelError::Parse("Request body is empty".to_string()));
    }
    ...
    let req: EditImageParameters = serde_json::from_slice(body.as_ref().unwrap())
        .map_err(|e| ModelError::Parse(e.to_string()))?;
    match ModelFactory::get_provider(&req.model.clone().unwrap_or_default()) {
        Ok(provider) => Proxy::edit_image(req, &provider).await,
        Err(e) => Err(e),
    }
}
