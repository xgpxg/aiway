...
pub async fn routing_handle(session: &mut Session, context: &mut HttpContext) -> HttpResult<()> {
    // 匹配路由
    let router = ROUTER
        .get()
        .ok_or(HttpError::new(500, "Router not initialized"))?;
    ...
    let route = router
        ...
        .ok_or(HttpError::new(404, "Not Found"))?;
