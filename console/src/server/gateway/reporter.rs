...
pub async fn report(req: State) -> anyhow::Result<()> {
    ...
    let gateway_state_log = GatewayNodeStateBuilder::default()
        ...
        .cpu_usage(req.system_state.cpu.usage)
        .mem_total(req.system_state.mem.total)
        .mem_free(req.system_state.mem.free)
        .mem_used(req.system_state.mem.used)
        .disk_total(req.system_state.disk.total)
        .disk_free(req.system_state.disk.free)
        .net_rx(req.system_state.net.rx)
        .net_tx(req.system_state.net.tx)
        .net_tcp_conn_count(req.system_state.net.tcp_conn_count)
        .gpu_state(req.system_state.gpu)
