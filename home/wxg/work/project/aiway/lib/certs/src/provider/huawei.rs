...
impl HuaweiProvider {
    ...
    async fn get_zone_id(&self, domain: &str) -> Result<String, CertError> {
        ...
        for i in 0..parts.len().saturating_sub(1) {
            ...
            let resp = self
                .call_api("GET", &format!("/v2/zones?name={}&limit=1", url_encode(&name)), None)
                .await?;
    ...
    /// 获取 TXT 记录集 ID
    async fn get_recordset_id(&self, zone_id: &str, name: &str) -> Result<Option<String>, CertError> {
        let resp = self
            .call_api(
                "GET",
                &format!("/v2/zones/{}/recordsets?name={}&type=TXT&limit=1", zone_id, url_encode(name)),
                None,
            )
            .await?;
        Ok(resp["recordsets"]
    ...
    async fn call_api(
        ...
        let mut req = self.client
            .request(reqwest::Method::from_bytes(method.as_bytes()).map_err(|_| {
        ...
        if !status.is_success() {
            let msg = result["error_msg"].as_str()
...
impl DnsProviderOps for HuaweiProvider {
    ...
    async fn create_txt(&self, domain: &str, value: &str) -> Result<(), CertError> {
        ...
        self.call_api("POST", &format!("/v2/zones/{}/recordsets", zone_id), Some(body)).await?;
        Ok(())
    }
    ...
    async fn delete_txt(&self, domain: &str, _value: &str) -> Result<(), CertError> {
        ...
        if let Some(rs_id) = self.get_recordset_id(&zone_id, &name).await? {
            self.call_api("DELETE", &format!("/v2/zones/{}/recordsets/{}", zone_id, rs_id), None).await?;
        }
        Ok(())
    }
}
